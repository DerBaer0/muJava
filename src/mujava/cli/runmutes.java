...
public class runmutes {
  ...
  public static void main(String[] args) throws Exception {
	 ...
	 if (jct.getParameters().size() == 1) {
		...
		Util.loadConfig(testSessionName);
		...
		System.out.println("testset path: " + folder + " contains " + listOfFiles.size() + " files");
		for (File file : listOfFiles) {
		  String fileName = file.getCanonicalPath();
		  if (fileName.contains(".class") && !fileName.contains("$")) {
			 fileName = fileName.replace(".class", "");
			 fileName = fileName.substring(folderFile.getCanonicalPath().length() + 1);
			 fileName = fileName.replace("/", ".");
			 testSetList.add(fileName);
			 System.out.println("adding " + fileName);
		  } else {
		  	System.out.println("not adding " + fileName);
		  }
 	...
 	try {
		...
		MutationSystem.addURL(MutationSystem.CLASS_PATH);
		String paths [] = Util.extraCP.split(":");
		Util.Error("Classpath: found extra " + Util.extraCP);
		for (String path : paths) {
			MutationSystem.addURL(path);
		}
	} catch (Exception e) {
		Util.Error("Mutation System: An error occured");
	}
	...
	String sessionFolder = muJavaHomePath + "/" + testSessionName;
	String basePath = new File(sessionFolder + "/classes/").getCanonicalPath();
	...
	for (int i = 0; i < listOfFilesInSession.size(); i++) {
		...
		if (temp.endsWith(".class") && !temp.contains("$")) {
			System.out.println("Adding " + temp.substring(basePath.length() + 1));
			fileList.add(temp.substring(basePath.length() + 1));
		}
	}
...
System.out.println("handling class " + targetClassName + ", isSingle = " + isSingleTestSet + ", size " + testSetList.size());
		if (isSingleTestSet)
  ...
  static void runTests(String targetClassName, String testSetName, String[] mutantTypes,
	...
	// read file
	 String location = MutationSystem.MUTANT_HOME + "/" + targetClassName.replace("/", ".") + "/" + MutationSystem.TM_DIR_NAME;

	 ArrayList<String> methodNameList = new ArrayList<>();
	ArrayList<String> classMutList = new ArrayList<>();
	// get all method names
	File folderTrad = new File(location);
	File[] listOfMethods = folderTrad.listFiles();

	if (listOfMethods == null) {
//		Util.Error("Skipping file " + targetClassName);
		return;
	} else {
//		Util.Error("NoSkip file " + targetClassName);
	}
	Util.Print("-----------------------------------------------");
	Util.Print("Class Name: " + targetClassName);
	Util.Print("Test Set Name: " + testSetName);
	Util.Print("Location: " + location);
	for (File mutant : listOfMethods) {
		methodNameList.add(mutant.getName());
	}
// get all class mutations
	File folderClass = new File(MutationSystem.MUTANT_HOME + "/" + targetClassName.replace("/", ".") + "/" + MutationSystem.TM_DIR_NAME);
	File[] listOfClassMuts = folderClass.listFiles();

	for (File mutant : listOfClassMuts) {
		classMutList.add(mutant.getName());
	}
	  ...
	  */
	  // FIXME: remove true
	 if (true || !methodNameList.contains("mutant_list") || mode.equals("fresh")) {
		// mode="fresh-default";
		Util.Print("Runmode: fresh");
		...
		Util.DebugPrint("Testset: " + targetClassName + " " + testSetName);
